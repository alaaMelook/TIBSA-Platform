# MODIFIED: FIX 2 | Fixed CWE reference mappings (XSS→CWE-79, Missing Auth→CWE-306, SQL→CWE-89, CSRF→CWE-352, Path Traversal→CWE-22, XXE→CWE-611, Insecure Deser→CWE-502, SSRF→CWE-918); added missing CAPEC entries referenced by THREAT_TITLE_TO_CAPEC
"""
...
# ─── Direct Threat-to-CAPEC Lookup ───────────────────────────────────────────
# Maps threat titles (as generated by _build_threats) to canonical CAPEC IDs.
# O(1) lookup; preferred over fuzzy keyword matching for accuracy and speed.
THREAT_TITLE_TO_CAPEC: Dict[str, str] = {
    "SQL / Query Injection":                           "CAPEC-66",
    "NoSQL Injection (MongoDB)":                       "CAPEC-261",
    "NoSQL Database Injection via External API":       "CAPEC-261",
    "Database Poisoning via Third-Party Integration":  "CAPEC-7",
    "Identity Spoofing":                               "CAPEC-194",
    "Missing Authentication Controls":                 "CAPEC-115",
    "MQTT Broker Unauthorized Access":                 "CAPEC-115",
    "Redis Unauthenticated Access":                    "CAPEC-115",
    "Privilege Escalation":                            "CAPEC-1",
    "Unrestricted Admin Panel Access":                 "CAPEC-1",
    "Broken Object-Level Authorization":               "CAPEC-1",
    "Cloud Misconfiguration":                          "CAPEC-1",
    "Container Escape":                                "CAPEC-1",
    "Flask Debug Mode in Production":                  "CAPEC-1",
    "Tenant Data Isolation Failure":                   "CAPEC-1",
    "Multi-Environment Configuration Drift":           "CAPEC-1",
    "Sensitive Data Exposure":                         "CAPEC-150",
    "Elasticsearch Open Cluster":                      "CAPEC-150",
    "Elasticsearch PII Exposure":                      "CAPEC-150",
    "Sensitive Data Leakage through Third-Party APIs": "CAPEC-150",
    "Cross-Site Scripting (XSS)":                      "CAPEC-86",
    "Missing HTTP Security Headers (Node)":            "CAPEC-86",
    "Cross-Site Request Forgery (CSRF)":               "CAPEC-62",
    "Unencrypted HTTP Traffic":                        "CAPEC-158",
    "Insecure FTP Credential Exposure":                "CAPEC-158",
    "WebSocket Hijacking":                             "CAPEC-111",
    "API Token Theft and Replay Attacks":              "CAPEC-60",
    "Third-Party API Compromise":                      "CAPEC-17",
    "Server-Side Request Forgery (SSRF) via REST APIs": "CAPEC-664",
    "GraphQL Query Complexity Attacks":                "CAPEC-197",
    "gRPC Service Reflection Exposure":                "CAPEC-169",
    "Serverless Function Event Injection":             "CAPEC-88",
    "PHP Remote Code Execution Risk":                  "CAPEC-88",
    "Edge / CDN Cache Poisoning":                      "CAPEC-141",
    "Mass Assignment Vulnerability":                   "CAPEC-212",
    "Memory Safety Vulnerabilities":                   "CAPEC-8",
    "Hardcoded Credentials in Firmware":               "CAPEC-70",
    "Django SECRET_KEY Exposure":                      "CAPEC-150",
    "Django DEBUG=True in Production":                 "CAPEC-150",
    "Insecure localStorage Token Storage":             "CAPEC-150",
    "Missing Content Security Policy (CSP)":           "CAPEC-86",
    "Spring Actuator Endpoint Exposure":               "CAPEC-150",
    "FastAPI Docs Exposure":                           "CAPEC-150",
    # Stack-aware threat titles (FIX 4)
    "DOM XSS / Client-Side Injection":                 "CAPEC-86",
    "Man-in-the-Middle via Unencrypted HTTP":          "CAPEC-94",
    "Sensitive Data Information Disclosure":           "CAPEC-150",
    "Broken Access Control on Admin Panel":            "CAPEC-1",
...
class CAPECEnrichmentService:
    ...
    def _load_patterns(self) -> Dict[str, CAPECPattern]:
        ...
        return {
            ...
            "CAPEC-86": CAPECPattern(
                ...
                description="An attacker embeds cross-site scripting payloads via HTTP query strings or NULL bytes to bypass validation and execute malicious scripts.",
                likelihood="High",
                severity="High",
                prerequisites=["Reflected user input in HTTP responses"],
                skills_required="Medium",
                resources_required=["XSS payload", "Web browser"],
                mitigations=[
            ...
            "CAPEC-115": CAPECPattern(
                ...
                description="An attacker bypasses authentication controls to gain unauthorized access to a system, API, or service that lacks proper authentication enforcement.",
                likelihood="High",
                severity="High",
                prerequisites=["Authentication is missing or improperly enforced"],
                skills_required="Medium",
                resources_required=["HTTP client or network tools"],
                mitigations=[
                    "Enforce authentication on all sensitive endpoints",
                    "Use a centralised authentication gateway",
                    "Implement multi-factor authentication (MFA)",
                    "Regularly audit endpoints for missing authentication"
                ],
                related_weaknesses=["CWE-306"],
            ...
            # ──────────────────────────────────────────────────────────────────
            # FIX 2: Missing entries referenced by THREAT_TITLE_TO_CAPEC.
            # Correct CWE IDs applied per spec:
            #   XSS → CWE-79, Missing Auth → CWE-306, SQL Inj → CWE-89,
            #   CSRF → CWE-352, Path Traversal → CWE-22, XXE → CWE-611,
            #   Insecure Deser → CWE-502, SSRF → CWE-918
            # ──────────────────────────────────────────────────────────────────
            "CAPEC-70": CAPECPattern(
                id="CAPEC-70",
                name="Try Common or Default Usernames and Passwords",
                description="An attacker exploits devices that use hardcoded or default credentials (username/password pairs) by trying known common values to gain unauthorized access.",
                likelihood="High",
                severity="High",
                prerequisites=["Device or service exposes an authentication interface"],
                skills_required="Low",
                resources_required=["List of common/default credentials"],
                mitigations=[
                    "Change all default credentials at provisioning time",
                    "Generate unique credentials per device",
                    "Enforce strong password policies",
                    "Implement account lockout after failed attempts"
                ],
                related_weaknesses=["CWE-1392", "CWE-521", "CWE-798"],
                stride_categories=[STRIDECategory.SPOOFING]
            ),
            "CAPEC-111": CAPECPattern(
                id="CAPEC-111",
                name="JSON Hijacking (aka JavaScript Hijacking)",
                description="An attacker exploits the lack of proper origin validation on WebSocket or JSON endpoints to hijack communication channels and inject or read messages.",
                likelihood="Medium",
                severity="High",
                prerequisites=[
                    "WebSocket or JSON endpoint without proper origin validation",
                    "Victim browser with same-origin policy weaknesses"
                ],
                skills_required="Medium",
                resources_required=["Malicious web page"],
                mitigations=[
                    "Validate the Origin header on every WebSocket upgrade",
                    "Use WSS (TLS) exclusively",
                    "Require authentication tokens at connection time",
                    "Implement CSRF tokens for WebSocket upgrade requests"
                ],
                related_weaknesses=["CWE-346", "CWE-942"],
                stride_categories=[STRIDECategory.TAMPERING, STRIDECategory.INFORMATION_DISCLOSURE]
            ),
            "CAPEC-115": CAPECPattern(
                id="CAPEC-115",
                name="Authentication Bypass",
                description="An attacker gains access to an application, service, or device with the same privileges as a legitimate user without providing valid authentication credentials. Missing or improperly implemented authentication allows this attack.",
                likelihood="High",
                severity="High",
                prerequisites=[
                    "Application exposes functionality without requiring authentication",
                    "Authentication mechanism is absent, incomplete, or bypassable"
                ],
                skills_required="Low",
                resources_required=["HTTP client or browser"],
                mitigations=[
                    "Implement authentication on all sensitive endpoints",
                    "Use OAuth 2.0 / OpenID Connect for standardised authentication",
                    "Apply server-side session validation on every request",
                    "Enforce MFA for privileged operations"
                ],
                related_weaknesses=["CWE-306", "CWE-287", "CWE-285"],
                stride_categories=[STRIDECategory.SPOOFING, STRIDECategory.ELEVATION_OF_PRIVILEGE]
            ),
            "CAPEC-141": CAPECPattern(
                id="CAPEC-141",
                name="Cache Poisoning",
                description="An attacker exploits the caching infrastructure (CDN, proxy, or edge node) to inject malicious content into cached responses, which is subsequently served to all users requesting that resource.",
                likelihood="Medium",
                severity="High",
                prerequisites=[
                    "Caching layer present (CDN, reverse proxy, edge node)",
                    "Improper cache key configuration or unvalidated headers"
                ],
                skills_required="Medium",
                resources_required=["Crafted HTTP request"],
                mitigations=[
                    "Set strict Cache-Control and Vary headers",
                    "Purge cache after every deployment",
                    "Validate all headers used as cache keys",
                    "Use cache isolation per user where sensitive data is involved"
                ],
                related_weaknesses=["CWE-20", "CWE-693"],
                stride_categories=[STRIDECategory.TAMPERING, STRIDECategory.INFORMATION_DISCLOSURE]
            ),
            "CAPEC-150": CAPECPattern(
                id="CAPEC-150",
                name="Collect Data from Common Resource Locations",
                description="An attacker accesses or monitors common resource locations (debug pages, configuration files, environment variables, log files) to collect sensitive data such as credentials, tokens, PII, or internal system details.",
                likelihood="High",
                severity="High",
                prerequisites=[
                    "Sensitive data stored or exposed through predictable locations",
                    "Insufficient access controls on debug or configuration endpoints"
                ],
                skills_required="Low",
                resources_required=["Web browser or HTTP client"],
                mitigations=[
                    "Disable debug endpoints and verbose error pages in production",
                    "Store secrets in a secrets manager (not source code or env files)",
                    "Apply access controls on all configuration and monitoring endpoints",
                    "Mask sensitive fields in application logs"
                ],
                related_weaknesses=["CWE-200", "CWE-312", "CWE-530"],
                stride_categories=[STRIDECategory.INFORMATION_DISCLOSURE]
            ),
            "CAPEC-158": CAPECPattern(
                id="CAPEC-158",
                name="Sniffing Network Traffic",
                description="An attacker passively observes network traffic to capture sensitive information transmitted in cleartext, including credentials, session tokens, and PII. Plain HTTP, FTP, and MQTT without TLS are common targets.",
                likelihood="Medium",
                severity="High",
                prerequisites=[
                    "Data transmitted over an unencrypted protocol (HTTP, FTP, MQTT)",
                    "Attacker has network access (same LAN, Wi-Fi, or ISP position)"
                ],
                skills_required="Low",
                resources_required=["Packet sniffer (Wireshark, tcpdump)"],
                mitigations=[
                    ...
                    "Enable TLS on MQTT brokers"
                ],
                related_weaknesses=["CWE-319", "CWE-311"],
                stride_categories=[STRIDECategory.INFORMATION_DISCLOSURE, STRIDECategory.TAMPERING]
            ),
            "CAPEC-169": CAPECPattern(
                id="CAPEC-169",
                name="Footprinting",
                description="An attacker enumerates the target’s exposed services and interfaces (e.g. gRPC server reflection, API docs) to map its attack surface and identify exploitable endpoints.",
                likelihood="High",
                severity="Low",
                prerequisites=["Service exposes metadata or reflection endpoints without authentication"],
                skills_required="Low",
                resources_required=["Network scanner or gRPC client"],
                mitigations=[
                    "Disable gRPC server reflection in production",
                    "Restrict access to API documentation endpoints",
                    "Enforce authentication on all service discovery interfaces"
                ],
                related_weaknesses=["CWE-200"],
                stride_categories=[STRIDECategory.INFORMATION_DISCLOSURE]
            ),
            "CAPEC-194": CAPECPattern(
                id="CAPEC-194",
                name="Fake the Source of Data",
                description="An attacker forges the apparent origin of data (identity spoofing) to deceive a target system into believing the data originates from a trusted source, enabling unauthorized access or manipulation.",
                likelihood="Medium",
                severity="High",
                prerequisites=["System trusts source identification without sufficient verification"],
                skills_required="Medium",
                resources_required=["Ability to craft forged requests or tokens"],
                mitigations=[
                    "Implement strong authentication (MFA, mutual TLS)",
                    "Validate source identity at every trust boundary",
                    "Use cryptographically signed tokens with short expiry"
                ],
                related_weaknesses=["CWE-290", "CWE-345"],
                stride_categories=[STRIDECategory.SPOOFING]
            ),
            "CAPEC-197": CAPECPattern(
                id="CAPEC-197",
                name="Exponential Data Expansion",
                description="An attacker submits specially crafted input that causes the application to expand data exponentially, consuming excessive memory or CPU. GraphQL deeply nested queries and XML Billion Laughs attacks are examples.",
                likelihood="Medium",
                severity="High",
                prerequisites=["Application processes recursive or nested data structures without limits"],
                skills_required="Medium",
                resources_required=["HTTP client"],
                mitigations=[
                    "Enforce query depth and complexity limits on GraphQL endpoints",
                    "Implement request size limits",
                    "Use query whitelisting for production GraphQL APIs",
                    "Monitor and alert on abnormal resource consumption"
                ],
                related_weaknesses=["CWE-400", "CWE-776"],
                stride_categories=[STRIDECategory.DENIAL_OF_SERVICE]
            ),
            "CAPEC-212": CAPECPattern(
                id="CAPEC-212",
                name="Functionality Misuse",
                description="An attacker exploits mass assignment vulnerabilities by supplying additional parameters that are automatically bound to model fields, allowing modification of fields the developer did not intend to expose.",
                likelihood="Medium",
                severity="High",
                prerequisites=["Framework binds request parameters directly to model objects without allowlisting"],
                skills_required="Low",
                resources_required=["HTTP client"],
                mitigations=[
                    "Use strong parameters / allowlisting in all frameworks",
                    "Explicitly declare which fields are user-settable",
                    "Validate and reject unexpected parameters"
                ],
                related_weaknesses=["CWE-915"],
                stride_categories=[STRIDECategory.TAMPERING, STRIDECategory.ELEVATION_OF_PRIVILEGE]
            ),
            "CAPEC-261": CAPECPattern(
                id="CAPEC-261",
                name="Fuzzing for application mapping",
                description="An attacker injects MongoDB query operators (e.g. $where, $regex, $ne) into unsanitized user input to manipulate NoSQL database queries, bypass authentication, or exfiltrate data from MongoDB collections.",
                likelihood="High",
                severity="High",
                prerequisites=[
                    "Application uses MongoDB with user-controlled query parameters",
                    "Input is not sanitized before constructing MongoDB queries"
                ],
                skills_required="Medium",
                resources_required=["HTTP client with JSON body manipulation"],
                mitigations=[
                    "Sanitize inputs to remove MongoDB operators ($where, $regex, $gt, etc.)",
                    "Use Mongoose with strict schema validation",
                    "Disable the $where operator in MongoDB configuration",
                    "Apply least privilege on MongoDB database accounts"
                ],
                related_weaknesses=["CWE-943", "CWE-89"],
                stride_categories=[STRIDECategory.TAMPERING, STRIDECategory.INFORMATION_DISCLOSURE]
            ),
            "CAPEC-664": CAPECPattern(
                id="CAPEC-664",
                name="Server Side Request Forgery",
                description="An attacker induces the server to make HTTP requests to an arbitrary internal or external URL by supplying a crafted URL in user-controlled input, enabling port scanning, SSRF-based data exfiltration, or access to internal services.",
                likelihood="High",
                severity="High",
                prerequisites=[
                    "Application fetches URLs derived from user-supplied input",
                    "No URL allowlisting or DNS rebinding protection in place"
                ],
                skills_required="Medium",
                resources_required=["HTTP client"],
                mitigations=[
                    "Validate and allowlist all outbound URLs before making requests",
                    "Block requests to private IP ranges (RFC 1918) and loopback",
                    "Implement DNS rebinding protection",
                    "Use request timeouts and enforce egress firewall rules"
                ],
                related_weaknesses=["CWE-918", "CWE-441"],
                stride_categories=[STRIDECategory.INFORMATION_DISCLOSURE, STRIDECategory.ELEVATION_OF_PRIVILEGE]
            ),
        }

    def get_capec_for_threat(self, threat_title: str) -> Optional[CAPECPattern]:
        """
        Direct O(1) lookup: returns the CAPECPattern for a known threat title.
        Returns None if the title is not in the lookup table or the CAPEC ID
        is not present in our local patterns database.
        """
        capec_id = THREAT_TITLE_TO_CAPEC.get(threat_title)
        if capec_id:
            return self.patterns.get(capec_id)   # None if not in local DB
        return None
    ...
    def enrich_threat(self, threat: ThreatItem) -> ThreatItem:
        ...
        Priority: direct title lookup → keyword fallback.
        IMPORTANT: never overwrites asvs_controls; only sets capec_id and
        annotates the description with CWE references when available.
        """
        # 1. Try direct title lookup first (accurate, fast)
        pattern = self.get_capec_for_threat(threat.title)

        # 2. Fall back to keyword matching if no direct hit
        if pattern is None:
            for p in self.patterns.values():
                if p.matches_threat(threat):
                    pattern = p
                    break

        if pattern is None:
            # Set capec_id from table even if full pattern is absent locally
            capec_id = THREAT_TITLE_TO_CAPEC.get(threat.title)
            if capec_id:
                threat.capec_id = capec_id
            return threat

        # Set CAPEC ID
        threat.capec_id = pattern.id

        # Annotate description with CWE references (do NOT touch asvs_controls)
        if pattern.related_weaknesses:
            cwe_refs = ", ".join(pattern.related_weaknesses[:3])
            if "[CWE:" not in threat.description:
