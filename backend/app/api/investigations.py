...
async def list_investigations(
    ...
    auth_user = current_user["auth_user"]
    try:
        resp = supabase.table("investigations").select("id, scan_id, target, status, risk_score, started_at, completed_at, current_stage, progress_percent").eq("user_id", auth_user.id).order("started_at", desc=True).execute()
...
async def get_investigation(
    ...
    auth_user = current_user["auth_user"]
    resp = supabase.table("investigations").select("*").eq("id", id).eq("user_id", auth_user.id).execute()
    if not resp.data:
        raise HTTPException(
...
async def get_investigation_status(
    ...
    auth_user = current_user["auth_user"]
    resp = supabase.table("investigations").select("id, scan_id, target, status, risk_score, started_at, completed_at, current_stage, progress_percent").eq("id", id).eq("user_id", auth_user.id).execute()
    if not resp.data:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Investigation not found."
...
async def get_investigation_findings(
    ...
    auth_user = current_user["auth_user"]
    resp = supabase.table("investigations").select("pipeline_state").eq("id", id).eq("user_id", auth_user.id).execute()
    if not resp.data:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Investigation not found."
...
async def stop_investigation(
    ...
    try:
        ...
        resp = supabase.table("investigations").select("status, pipeline_state, current_stage").eq("id", id).eq("user_id", auth_user.id).execute()
        if not resp.data:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="Investigation not found."
