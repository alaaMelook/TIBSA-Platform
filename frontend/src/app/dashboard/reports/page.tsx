...
interface ScanReport {
    ...
    indicators: Array<{ type: string; value: string; threat_level: string } | string>;
    created_at: string;
}
    ...
    if (report.indicators?.length > 0) {
        ...
        for (const ind of report.indicators) {
            if (typeof ind === "string") {
                ...
                lines.push(`  [${(ind.threat_level || "info").toUpperCase()}] ${ind.type}: ${ind.value}`);
            }
        }
        lines.push("");
    }
                                        ...
                                        <div>
                                            ...
                                            <div className="space-y-2">
                                                {selectedReport.indicators.map((ind, i) => {
                                                    ...
                                                    const type = isString ? "SIGNAL" : ind.type;
                                                    const value = isString ? ind : ind.value;
                                                    const threat = isString ? "info" : (ind.threat_level || "info");
                                                    return (
                                                        <div key={i} className="flex items-center justify-between bg-white/[0.04] p-3 rounded-lg text-sm border border-white/[0.06]">
                                                            ...
                                                            {!isString && (
                                                                <span className={`font-medium capitalize ${threatColor(threat)}`}>{threat}</span>
                                                            )}
                                                        </div>
                                                    );
                                                })}
                                    ...
                                    {tmList.map(tm => (
                                        ...
                                        >
                                            <div className="flex items-center justify-between">
                                                ...
                                                <span className={`text-xs font-medium px-2 py-0.5 rounded-full border ${riskBadge(tm.risk_label)}`}>
                                                    {tm.risk_label}
                                                </span>
                                            </div>
