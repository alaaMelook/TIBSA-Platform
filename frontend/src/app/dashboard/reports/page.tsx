...
function buildTextReport(scan: Scan, report: ScanReport): string {
    const divider = "ΓòÉ".repeat(60);
    const thinDiv = "ΓöÇ".repeat(60);
    const lines: string[] = [];

    lines.push(divider);
    lines.push("  TIBSA ΓÇö Threat Intelligence Security Report");
    lines.push(divider);
    lines.push("");
    ...
    if (malice && !malice.error) {
        ...
        if (engines.length > 0) {
            ...
            lines.push(`  ${"ΓöÇ".repeat(20)} ${"ΓöÇ".repeat(12)} ${"ΓöÇ".repeat(24)}`);
            for (const eng of engines) {
                ...
                const res = isMal ? String(eng.result || "Malware") : (eng.error ? String(eng.error) : "ΓÇö");
    ...
    lines.push(divider);
    lines.push(`  Generated by TIBSA Platform ΓÇö ${new Date().toLocaleString()}`);
    lines.push(divider);
                                        ...
                                        <div className="absolute right-0 top-full mt-1 w-52 rounded-lg bg-[#1a2744] border border-white/[0.08] shadow-2xl shadow-black/40 overflow-hidden z-10">
                                            ...
                                            >
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-[10px] text-slate-500">.txt ΓÇö human-readable</p>
                                                </div>
                                            </button>
                                            <button
                                            ...
                                            >
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-[10px] text-slate-500">.json ΓÇö machine-readable</p>
                                                </div>
                                            </button>
                                        </div>
                                    )}
