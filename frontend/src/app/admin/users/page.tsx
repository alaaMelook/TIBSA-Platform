...
export default function UsersManagementPage() {
    ...
    const columns: Column<AdminUser>[] = [
                ...
                return (
                    <div className="flex items-center gap-3">
                        <div className="relative flex-shrink-0 w-8 h-8 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center">
                            <span className="text-xs font-bold text-[var(--text-primary)]">{user.full_name.charAt(0)}</span>
                            {isOnline && (
        ...
        },
        {
            ...
            sortable: true,
            render: (user) => (
                <span className="text-sm text-[var(--text-secondary)] tabular-nums">{user.total_scans.toLocaleString()}</span>
            ),
        },
        {
            key: "threats_found",
            ...
            sortable: true,
            render: (user) => (
                <span className={`text-sm tabular-nums ${user.threats_found > 100 ? "text-red-400" : "text-[var(--text-secondary)]"}`}>
                    {user.threats_found}
                </span>
            ),
        },
        {
            ...
            sortable: true,
            render: (user) => (
                <span className="text-xs text-[var(--text-muted)]">{timeAgo(user.last_login)}</span>
            ),
        },
        {
            key: "actions",
            ...
            render: (user) => (
                <div className="flex items-center gap-1">
                    <button
                        ...
                        className="px-2.5 py-1 text-xs rounded-md bg-[var(--bg-elevated)] border border-[var(--border-soft)] text-[var(--text-secondary)] hover:bg-[var(--bg-elevated)] hover:text-[var(--text-primary)] transition-colors"
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.4 }} className="space-y-6 max-w-[1400px]">
            ...
            <div className="flex items-center justify-between flex-wrap gap-4">
                <div>
                    <div className="flex items-center gap-3 mb-1">
                        ...
                        <span className="px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-widest bg-gradient-to-r from-blue-500/20 to-[var(--primary-hover)]/20 border border-[var(--primary)] text-[var(--primary)] rounded-full">
                            Admin
                        </span>
                    </div>
                    <p className="text-sm text-[var(--text-muted)]">Manage user accounts, roles, and permissions</p>
                </div>
            ...
            {selectedUser && (
                <div className="fixed inset-0 z-50 flex justify-end" onClick={() => setSelectedUser(null)}>
                    ...
                    <div
                        className="relative w-full max-w-md bg-[var(--bg-elevated)] border-l border-[var(--border-soft)] shadow-2xl overflow-y-auto animate-in slide-in-from-right duration-300"
                        onClick={(e) => e.stopPropagation()}
                    >
                        {/* Header */}
                        <div className="sticky top-0 bg-[var(--bg-elevated)] backdrop-blur-lg border-b border-[var(--border-strong)] px-6 py-4 flex items-center justify-between z-10">
                            <h3 className="text-lg font-semibold text-[var(--text-primary)]">User Details</h3>
                            <button
                        ...
                        <div className="px-6 py-6 space-y-6">
                            ...
                            <div className="flex items-center gap-4">
                                <div className="relative w-14 h-14 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center shadow-lg">
                                    <span className="text-xl font-bold text-[var(--text-primary)]">{selectedUser.full_name.charAt(0)}</span>
                                ...
                                <div>
                                    <div className="flex items-center gap-2">
                                        <p className="text-lg font-semibold text-[var(--text-primary)]">{selectedUser.full_name}</p>
                                        {(selectedUser.is_online ?? (selectedUser.last_login ? (Date.now() - new Date(selectedUser.last_login).getTime() <= 30000) : false)) && (
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                {[
                                    { label: "Total Scans", value: selectedUser.total_scans.toLocaleString(), color: "text-[var(--primary)]" },
                                    { label: "Threats Found", value: selectedUser.threats_found.toString(), color: "text-red-400" },
                                    { label: "Storage Used", value: `${selectedUser.storage_used} MB`, color: "text-[var(--primary)]" },
                                    { label: "Last Login", value: timeAgo(selectedUser.last_login), color: "text-amber-400" },
                                ].map((stat) => (
                                    <div key={stat.label} className="bg-[var(--bg-elevated)] rounded-lg border border-[var(--border-strong)] p-3">
                            ...
                            <div className="space-y-3">
                                <div className="flex items-center justify-between py-2 border-b border-[var(--border-soft)]">
                                    ...
                                    <span className="text-xs text-[var(--text-secondary)]">{new Date(selectedUser.created_at).toLocaleDateString()}</span>
                                </div>
                                <div className="flex items-center justify-between py-2 border-b border-[var(--border-soft)]">
                                    <span className="text-xs text-[var(--text-muted)]">Last Updated</span>
                                    <span className="text-xs text-[var(--text-secondary)]">{new Date(selectedUser.updated_at).toLocaleDateString()}</span>
                                </div>
                                <div className="flex items-center justify-between py-2">
                                    <span className="text-xs text-[var(--text-muted)]">User ID</span>
                                    <span className="text-xs text-[var(--text-muted)] font-mono">{selectedUser.id}</span>
                                </div>
                            </div>
                            ...
                            <div className="space-y-2 pt-2">
                                <button
                                    ...
                                    className="w-full px-4 py-2.5 text-sm rounded-lg bg-[var(--primary-soft)] border border-[var(--primary)] text-[var(--primary)] hover:bg-[var(--primary-soft)] transition-colors"
                                >
            ...
            {onlineDetailsOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={() => setOnlineDetailsOpen(false)}>
                    ...
                    <div
                        className="relative w-full max-w-lg bg-[#13203c]/95 border border-[var(--border-soft)] rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[80vh] animate-in zoom-in-95 duration-200"
                        onClick={(e) => e.stopPropagation()}
                    >
                        ...
                        {/* Header */}
                        <div className="px-6 py-5 border-b border-[var(--border-strong)] flex items-center justify-between">
                            <div>
                                ...
                                <p className="text-xs text-[var(--text-muted)] mt-1">Real-time analyst & user presence inside TIBSA SOC</p>
                            </div>
                            <button
                        ...
                        <div className="px-6 py-3 bg-emerald-500/[0.03] border-b border-[var(--border-soft)] flex items-center justify-between text-xs text-[var(--text-muted)]">
                            <span>Total Online: <strong className="text-emerald-400 font-semibold">{onlineCount}</strong></span>
                            <span>Active History: <strong className="text-[var(--primary)] font-semibold">{sortedOnlineAndRecent.length} accounts</strong></span>
                        </div>
                        ...
                        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-3 custom-scrollbar">
                            {sortedOnlineAndRecent.length === 0 ? (
                                <div className="py-8 text-center">
                                    <p className="text-sm text-[var(--text-muted)]">No active or past sessions tracked.</p>
                                </div>
                            ) : (
                                sortedOnlineAndRecent.map((user) => {
                                    ...
                                    return (
                                        <div 
                                            key={user.id}
                                            className="flex items-center justify-between p-3 rounded-xl bg-[var(--bg-elevated)] border border-[var(--border-soft)] hover:bg-[var(--bg-elevated)] hover:border-[var(--border-soft)] transition-all duration-200"
                                        >
                                            <div className="flex items-center gap-3">
                                                <div className="relative flex-shrink-0 w-9 h-9 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center font-bold text-[var(--text-primary)] shadow-inner">
                                                ...
                                                <div>
                                                    <div className="flex items-center gap-1.5">
                                                        <span className="text-sm font-semibold text-[var(--text-primary)]">{user.full_name}</span>
                                                        {isOnline ? (
                                                            ...
                                                            <span className="px-1.5 py-0.5 rounded text-[8px] font-bold bg-[var(--bg-elevated)] text-[var(--text-muted)] border border-[var(--border-soft)] uppercase tracking-wider">
                                                                offline
                                                            </span>
                                                        )}
                                                    </div>
                                                    <span className="text-xs text-[var(--text-muted)] block leading-tight">{user.email}</span>
                                                </div>
                                            </div>
                                            ...
                                            <div className="flex flex-col items-end gap-1">
                                                ...
                                                <span className="text-[10px] text-[var(--text-muted)] flex items-center gap-1">
                                                    <svg className="w-3.5 h-3.5 text-[var(--text-muted)] animate-pulse" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                        ...
                        {/* Footer */}
                        <div className="px-6 py-4 border-t border-[var(--border-strong)] bg-[#0c1426]/50 flex justify-end">
                            <button
                                onClick={() => setOnlineDetailsOpen(false)}
                                className="px-4 py-2 text-xs font-semibold rounded-lg bg-[var(--bg-elevated)] border border-[var(--border-soft)] text-[var(--text-secondary)] hover:bg-[var(--bg-elevated)] hover:text-[var(--text-primary)] transition-colors cursor-pointer"
                            >
                                Close Tracker
                            </button>
                        </div>
