  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-3 gap-3">
        <StatBox label="Rules Evaluated" value={String(corr.rules_evaluated)} color="text-slate-200" />
                ...
                <div key={rule.rule_id} className={`p-4 rounded-xl border ${cfg.bg} ${cfg.border}`}>
                  <div className="flex items-start justify-between gap-4 mb-2">
                    ...
                    <div className="flex flex-col items-end gap-1 flex-shrink-0">
                      ...
                      <span className="text-[9px] text-slate-500 uppercase font-bold">{rule.relationship_type}</span>
                    </div>
                  </div>
                    ...
                    <div className="mt-3 space-y-1">
                      ...
                      {rule.evidence.map((ev, i) => (
                        <div key={i} className="flex items-start gap-2">
                          ...
                          <span className="text-[11px] text-slate-400 font-mono">{ev}</span>
                        </div>
                      ))}
                    </div>
                  )}
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
            {untriggered.map((rule) => (
              <div key={rule.rule_id} className="flex items-center gap-3 p-3 rounded-xl border border-white/[0.04] bg-slate-900/20">
                <div className="w-1.5 h-1.5 rounded-full bg-slate-700 flex-shrink-0" />
                <span className="text-[11px] text-slate-500">{rule.rule_name}</span>
              </div>
            ))}
          </div>
        </div>
...
function StatBox({ label, value, color }: { label: string; value: string; color: string }) {
  return (
    <div className="flex flex-col items-center justify-center p-4 rounded-xl border border-white/[0.06] bg-[#1e293b]/60">
